...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($tugasBaru)) {
        ...
        if (isset($_SESSION['edit_index'])) {
            ...
            $_SESSION['tugas'][$index] = $tugasBaru; // Update tugas yang ada
...
<body>
    <div class="container">
        <h1>Aktivitas Geisbert 🚀</h1>
        <form name="Aktivitas-Geisbert" action="#" method="post">
            <input type="text" placeholder="Buat Kegiatanmu Geisbert..." name="tugas" value="<?php echo isset($_SESSION['edit_index']) ? htmlspecialchars($_SESSION['tugas'][$_SESSION['edit_index']]) : ''; ?>" required>
        ...
        <?php if (isset($_SESSION['tugas']) && !empty($_SESSION['tugas'])) : ?>
            <ol>
                <?php foreach ($_SESSION['tugas'] as $index => $tugas) : ?>
                    <li>
                        <span><?php echo htmlspecialchars($tugas); ?></span>
                        <div class="actions">
